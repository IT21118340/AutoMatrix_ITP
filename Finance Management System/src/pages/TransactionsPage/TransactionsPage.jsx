...
export default function TransactionsPage() {
  ...
  useEffect(() => {
    async function fetchTransactions() {
      try {
        const [incomesData, expensesData,capitalData] = await Promise.all([
          incomesAPI.getIncomes(),
          expensesAPI.getExpenses(),
          capitalAPI.getCapital()
        ]);
...
const capitalTotal = filteredTransactions
  .filter((transaction) => transaction.category === 'Capital')
  .reduce((total, transaction) => total + transaction.amount, 0);

const cashFlow = capitalTotal + incomeTotal - expensesTotal;
