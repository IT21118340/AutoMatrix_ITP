...
export class CreateTicket extends Component {
  ...
  onSubmit(e) {
    ...
    if (this.state.name.length < 5) {
      this.setState({
        nameError: 'Student Name should be longer than 3 characters.',
      })
    } else if (this.state.id.length < 4) {
      this.setState({ idError: 'Invalid ID.' })
    } else if (this.state.description.length < 10) {
      this.setState({
        descriptionError: 'Student Name should be longer than 5 words.',
      })
      ...
      axios.post('http://localhost:80/ticket/add', ticket).then((res) => {
        console.log(res)
        if (res.status === 200) {
          this.clearData()
          Swal.fire({
